...
if __name__ == '__main__':
    ...
    parser.add_argument('-eu', '--euclid',  action='store_true')
    ...
    arrow_width = 0.75
    p1 = [(1.7, -np.pi*0.36), (1.9, -np.pi*0.28), (1.9, 0), (1.8, 0.28*np.pi),  (1.7, 0.36*np.pi)]
    ...
    if args.task == 'hmon':
        bench_cls = HMONBench
        exp_kwargs = {'split': 'train', 'num_scenes': 100, 'num_episodes_per_scene': 2}
        run_kwargs['success_thresh'] = args.success_thresh
        run_kwargs['euclid'] = True  if args.euclid else False 
        run_kwargs['success_thresh'] = 0.1

    if args.task == 'goat':
        bench_cls = GOATBench
        exp_kwargs = {'split': args.split, 'num_scenes': 40}
    ...
    nbench = bench_cls(sim_kwargs=sim_kwargs, vlm_agent=vlm_model, exp_kwargs=exp_kwargs, outer_run_name=outer_run_name, catch = True if args.catch else False, clip_mag=clip_mag)
